...
class Search_invoice extends MY_Controller {
    ...
    function index($flag) {
        ...
        $this->data['page_title'] = lang('search_invoice');
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $bc = array(array('link' => '#', 'page' => lang('search_invoice')));
        $meta = array('page_title' => lang('search_invoice'), 'bc' => $bc);
      ...
      // If captcha form is submitted
        ...
        if($inputCaptcha === $sessCaptcha){
            ...
            if(!$data){
                ...
                $arr = explode("/", $data[0]->file_name);
                $nombre = $arr[1];

                $filename = FCPATH.'uploads/pdf/' . $data[0]->file_name;
