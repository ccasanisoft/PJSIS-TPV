...
class Settings extends MY_Controller
{
    ...
    function index() {
		...
		$this->form_validation->set_rules('mail_incidents', lang('mail_incidents'), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'site_name' => DEMO ? 'SistemaTPV' : $this->input->post('site_name'),
                'tel' => $this->input->post('tel'),
                'language' => $this->input->post('language'),
                'currency_prefix' => DEMO ? 'PEN' : strtoupper($this->input->post('currency_prefix')),
				...
				'mail_incidents' => $this->input->post('mail_incidents'),//************TRJ050 RENATO GAMONAL //
                ...
                'nventa_format' => $this->input->post('nventa_format'), //*****Renato TRJ023 25/04/2019   ********
				...
				'advanced_sale' => $this->input->post('advanced_sale'),//****************TRJ044 - ALEXANDER ROCA -23/05/2019***********
				'tax_icbper' => $this->input->post('tax_ICBPER'),//****************TRJ068 - ALEXANDER ROCA -14/08/2019***********
                'pdf_format' => $this->input->post('pdf_format'), //*****************************TRJ088 - KENY PONTE - 02/09/2019********* */
				'type_Print' => $this->input->post('type_Print'),//****************TRJ098 - ALEXANDER ROCA -16/10/2019***********
            );
            ...
            if ($this->input->post('nventa_format')) {//*****Renato TRJ023 25/04/2019   ********
                $nventa_format = $this->input->post('nventa_format');//*****Renato TRJ023 25/04/2019   ********
                ...
                $buscar = strpos($parentesis,'+');
                if($buscar){
                    $numero = explode('+',$parentesis);//*****Renato TRJ023 25/04/2019   ********
                    $data['nventa_number'] = (int)$numero[1];//*****Renato TRJ023 25/04/2019   ********
                    $format = $parte1[0].'{'.$numero[0].'}';
                    $data['nventa_format'] = $format;//*****Renato TRJ023 25/04/2019   ********
                }
            }
